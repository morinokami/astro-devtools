...
- Dependency versions live in the pnpm catalogs (`pnpm-workspace.yaml`, strict mode): `dependencies`/`devDependencies` use `catalog:` (`catalog:devtools` for the devtools release train, `workspace:*` for workspace packages); only `peerDependencies` carry version literals.
...
- Inlined dependencies are an explicit contract: a dependency that gets bundled must be listed in `pack.deps.onlyBundle` (`packages/astro-devtools/vite.config.ts`), or the build fails. `@astro-devtools/ui` is outside this contract: it is consumed as TypeScript source (its `exports` point into `src/`, so it has no build step) and bundled directly, while its own bundled dependencies must still be listed there.
