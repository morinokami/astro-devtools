...
- `mcp-tools` lists `astro-devtools_overview_info`, `astro-devtools_routes_info` and `astro-devtools_actions_info`, each marked read-only, and none of the panel-only queries (plus upstream's own read-only `devframe_state_read`).
...
- **Tool list.** Run `drive.ts mcp tools --save mcp-tools`. The list contains `astro-devtools_overview_info`, `astro-devtools_routes_info` and `astro-devtools_actions_info`, each with `readOnlyHint: true` and a description ending in `This query is read-only.`; it does not contain `astro-devtools_project_context` or `astro-devtools_config_info` (upstream adds its own `devframe_state_read`, also `readOnlyHint: true`).
- **Call.** Run `drive.ts mcp call astro-devtools_overview_info --save mcp-overview`. `isError` is `false` and `data.astroVersion` is a semver; `drive.ts mcp call astro-devtools_actions_info` lists the five playground actions with `actionsFile: "src/actions/index.ts"`; `drive.ts mcp call astro-devtools_routes_info` has a `context` with `adapterName` and `output` and a non-empty `routes` array.
- **Parity.** Run `drive.ts browser rpc astro-devtools:overview:info --save parity-rpc` and compare with `mcp-overview.json`: the RPC result and the tool's `data` are identical JSON.
