...
- **Data behind it.** Run `drive.ts browser rpc astro-devtools:config:info --save config-rpc`. `configFile` is `astro.config.mjs`, `root` is the playground's absolute path with a trailing slash, and `entries` holds the same key/value pairs as the rows.
...
- This panel is not on the MCP surface (`astro-devtools:config:info` carries no agent metadata); prove it through the page or `browser rpc`, not `mcp call`.
